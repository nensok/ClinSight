...
import type { CostDistBucket } from "@/lib/types";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

interface Props { data: CostDistBucket[] }
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-semibold">Cost Distribution by Range</CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 20 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={grid} />
            <XAxis dataKey="range" tick={{ fontSize: 10, fill: text }} angle={-30} textAnchor="end" />
            <YAxis tick={{ fontSize: 11, fill: text }} />
            <Tooltip
              contentStyle={{ background: bg, border: `1px solid ${grid}`, borderRadius: 8, color: fg }}
              formatter={(v: unknown) => [`${Number(v).toLocaleString()} encounters`, "Count"]}
            />
            <Bar dataKey="count" fill="#f59e0b" name="Encounters" radius={[3,3,0,0]} />
          </BarChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
